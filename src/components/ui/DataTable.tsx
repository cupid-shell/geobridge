...
export const DataTable: React.FC<DataTableProps> = ({
  ...
  return (
    <div className="bg-white border border-slate-200/90 rounded-xl shadow-xs overflow-hidden flex flex-col">
      ...
      <div className="p-3 border-b border-slate-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 bg-slate-50/50">
        ...
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-slate-400 focus:border-slate-400"
          />
        </div>
      ...
      >
        <table className="w-full text-left border-collapse border-spacing-0 text-xs">
          <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-200 shadow-2xs">
            <tr>
              <th className="py-2.5 px-3 font-semibold text-slate-500 text-[11px] uppercase tracking-wider w-12 text-center border-r border-slate-100">
                #
              </th>
              {columns.map((col) => {
                ...
                return (
                  <th
                    ...
                    className={`py-2 px-3 font-semibold text-xs cursor-pointer select-none transition-colors border-r border-slate-100 ${
                      isEnriched
            ...
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((row, idx) => {
                ...
                return (
                  ...
                  >
                    <td className="py-2 px-3 font-mono text-[11px] text-slate-400 text-center border-r border-slate-100 tabular-nums">
                      {globalIndex}
                    </td>
                    {columns.map((col) => {
                      const val = row[col];
                      ...
                      return (
                        <td
                          key={col}
                          className={`py-2 px-3 border-r border-slate-100 truncate max-w-[200px] ${
                            isEnriched ? 'bg-slate-50/40 font-medium text-slate-900' : 'text-slate-700'
                          } ${isNumeric || isCoord ? 'tabular-nums font-mono text-[11px]' : 'text-xs'}`}
                          title={String(val ?? '')}
                        >
      ...
      <div className="px-4 py-2.5 bg-slate-50/70 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-500">
        <div className="flex items-center space-x-2">
          ...
          <select
            ...
            className="bg-white border border-slate-200 rounded px-2 py-1 text-xs font-mono text-slate-800"
          >
        ...
        <div className="flex items-center space-x-1.5">
          ...
          <span className="px-2 font-mono text-[11px] tabular-nums text-slate-700">
